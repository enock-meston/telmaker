...
include 'include/condig.php';
  ...
  if (isset($_POST['submitbtn1'])) {
     ...
     $pass2 = mysqli_real_escape_string($con,$_POST["REpassword"]); 
     if ($pass1 != $pass2) {
       $error =" passwords are not match!!";
     } else {
         $hashpassword=password_hash($pass1, PASSWORD_BCRYPT);
         $updatePassword = mysqli_query($con,"UPDATE `clienttbl` SET `password`='$hashpassword' 
         WHERE email='$email'");
                                    ...
                                    <form class="user" method="POST">
                                        <div class="form-group">
                                            <input type="password" name="password" class="form-control form-control-user"
                                               placeholder="Enter New Password" maxlength="15" required>
                                        </div>
